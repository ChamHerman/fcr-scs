...
describe("CompensationLedger", function () {
  let contract: any, owner: any, nonOwner: any;

  beforeEach(async function () {
    [owner, nonOwner] = await ethers.getSigners();
    contract = await (await ethers.getContractFactory("CompensationLedger")).deploy();
  });
  ...
  describe("publishRecord", function () {
    ...
    it("reverts for zero hash", async () => {
      await expect(contract.publishRecord("CASE-001", ethers.ZeroHash)).to.be.revertedWith("Invalid hash");
    });
  });
