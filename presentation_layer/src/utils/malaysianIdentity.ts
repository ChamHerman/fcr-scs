...
/**
 * Extracts digits from IC input (up to 12)
 */
export function parseRawIc(input: string | null | undefined): string {
  if (!input) return '';
  return String(input).replace(/\D/g, '').slice(0, 12);
}

/**
 * Formats 12 digits to NRIC format: XXXXXX-XX-XXXX
 */
export function formatIc(input: string | null | undefined): string {
  const digits = parseRawIc(input);
