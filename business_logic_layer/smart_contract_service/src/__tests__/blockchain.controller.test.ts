...
describe("POST /api/blockchain/publish — input validation", () => {
  it("400 when caseId is missing", async () => {
    const r = await request(app).post("/api/blockchain/publish")
...
describe("POST /api/blockchain/void — input validation", () => {
  it("400 when voidReason is missing", async () => {
    const r = await request(app).post("/api/blockchain/void")
      .send({ caseId: "CASE-001", walletAddress: "0xAdminWallet123" });
    expect(r.status).toBe(400);
  });
});

describe("POST /api/blockchain/verify — input validation", () => {
  it("400 when no file uploaded", async () => {
    const r = await request(app).post("/api/blockchain/verify");
    expect(r.status).toBe(400);
  });
...
describe("GET /api/blockchain/records/:caseId", () => {
  it("404 for unknown caseId", async () => {
    const r = await request(app).get("/api/blockchain/records/NONEXISTENT-999");
